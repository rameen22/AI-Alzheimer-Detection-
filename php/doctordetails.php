...
$message = ''; // Initialize an empty message variableif (isset($_POST['submit'])) {
    ...
    if (move_uploaded_file($_FILES["imagefile"]["tmp_name"], $target_file)) {
        ...
        // Set the path to your reference MRI image
        $reference_mri_path = 'mildDem15.jpg';

        // Call the Python script with image paths as arguments
        $command = escapeshellcmd("python similarity.py \"$image_path\" \"$reference_mri_path\"");
        $result = shell_exec($command);

        // Handle the result from the Python script
        if ($result !== null) {
            $similarity_score = floatval($result);
            ...
            if ($similarity_score > $threshold) {
                ...
                $message = "MRI similarity score does not meet the threshold.";
            }
        } else {
